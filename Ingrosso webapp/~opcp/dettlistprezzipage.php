...
if ($searchOn == 'true') {
    ...
    if ($fld == 'id' || $fld == 'listino' || $fld == 'prodotto' || $fld == 'nomeprodotto' || $fld == 'gruppo' || $fld == 'unitamisura' || $fld == 'prezzounitario' || $fld == 'annotazioni') {
...
switch ($examp) {
    ...
    case 10: // SELECT
        ...
        if ($start < 0)
            $start = 0;
        $SQL = "SELECT b.id as id, b.listino as listino, b.prodotto as prodotto, g.NomeGruppo as gruppo, ";
        ...
        $SQL .= "FROM cp_dettagliolistino b LEFT OUTER JOIN cp_prodotto p ON (p.id = b.prodotto) LEFT OUTER JOIN cp_gruppoprodotti g ON (g.id = p.gruppo)";
