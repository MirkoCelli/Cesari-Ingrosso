...
if($searchOn=='true') {
    ...
    if( $fld=='id' || $fld =='cliente' || $fld=='nomecliente' || $fld=='datafattura' || $fld=='numerofattura' ||
        $fld == 'numererazionefattura' || $fld == 'totaleimponibile' || $fld == 'riferimenti' || $fld == 'perc_iva' ||
        $fld=='impostaiva' || $fld == 'totalefattura' || $fld=='descrizione') {
        ...
        if ($fld == 'numerofattura') {
            $fld = 'b.numerofattura';
        }
        if ($fld == 'numerazionefattura') {
            $fld = 'b.numererazionefattura';
        }
        if ($fld == 'totaleimponibile') {
            $fld = 'b.totaleimponibile';
        }
        if ($fld == 'riferimenti') {
            $fld = 'b.riferimenti';
        }
...
switch ($examp) {
    ...
    case 10:  // SELECT
        $result = mysqli_query($db,"SELECT COUNT(*) AS count FROM cp_fattura b LEFT OUTER JOIN cp_cliente c ON (c.id = b.cliente) WHERE 1 = 1 ".$wh);
        if (!$result){
        ...
        if ($start<0) $start = 0;
        $SQL = "SELECT b.id as id, b.cliente as cliente, c.Denominazione as nomecliente, ";
        $SQL .= "b.datafattura as datafattura, b.numerofattura as numerofattura, b.numerazionefattura as numerazionefattura,";
        $SQL .= "b.totaleimponibile AS totaleimponibile, b.riferimenti as riferimenti, b.perc_IVA as perc_iva, b.impostaIVA as impostaiva,";
        $SQL .= "b.totalefattura AS totalefattura, b.descrizione as descrizione ";
        $SQL .=	"FROM cp_fattura b ";
        $SQL .= "LEFT OUTER JOIN cp_cliente c ON (c.id = b.cliente) ";
        ...
        while ($row = mysqli_fetch_array($result)){
            ...
            $s .= "<cell>" . xml_entities($row['datafattura']) . "</cell>";
            $s .= "<cell>" . xml_entities($row['numerofattura']) . "</cell>";
            $s .= "<cell>" . xml_entities($row['numerazionefattura']) . "</cell>";
            $s .= "<cell>" . xml_entities($row['totaleimponibile']) . "</cell>";
            $s .= "<cell>" . xml_entities($row['riferimenti']) . "</cell>";
            $s .= "<cell>" . xml_entities($row['perc_iva']) . "</cell>";
            $s .= "<cell>" . xml_entities($row['impostaiva']) . "</cell>";
            $s .= "<cell>" . xml_entities($row['totalefattura']) . "</cell>";
    ...
    case 45: // Completa listino con i prodotti nuovi (non presenti in listino) con prezzo a zero
        ...
        $sql .= "SELECT listino, prodotto, unitamisura, prezzounitario FROM (SELECT " . $listino . " AS listino,  p.id AS prodotto,  p.unitamisura AS unitamisura, ";
        $sql .= "0.00 AS prezzounitario, l.id AS dalistino, p.sequenza AS sequenza \n";
        $sql .= "FROM cp_prodotto p LEFT OUTER JOIN cp_dettagliolistino l ON (l.listino = " . $listino . " AND l.prodotto = p.id) \n";
        $sql .= "HAVING dalistino IS NULL) prz ORDER BY sequenza ";
