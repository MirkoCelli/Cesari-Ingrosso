...
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
$wh = "";
$idfatturakey = $_REQUEST["idfattura"];

$searchOn = Strip($_REQUEST['_search']);
if ($searchOn == 'true') {
    $fld = Strip($_REQUEST['searchField']);
    if ($fld == 'id' || $fld == 'fattura' || $fld == 'gruppo' || $fld == 'nomegruppo' || $fld == 'totale' || $fld == 'bolla' || $fld == 'dettagliobolla' ||
        $fld == 'prodotto' || $fld == 'nomeprodotto' || $fld == 'quantita' || $fld == 'prezzounitario' || $fld == 'annotazioni') {
...
switch ($examp) {
    ...
    case 10: // SELECT
        if (isset($idfatturakey)) {
            $wh .= " AND fattura = $idfatturakey ";
        }
        $result = mysqli_query($db, "SELECT COUNT(*) AS count FROM cp_dettagliofattura b WHERE 1 = 1 " . $wh);
        ...
        if ($start < 0)
            $start = 0;
        $SQL = "SELECT b.id as id, b.fattura as fattura, b.gruppo as gruppo, g.NomeGruppo as nomegruppo, b.prodotto as prodotto, ";
        $SQL .= "p.descrizionebreve as nomeprodotto, b.quantita as quantita, b.prezzounitario as prezzounitario, b.totale as totale, b.descrizione as descrizione, ";
        $SQL .= "b.bolla as bolla, b.dettagliobolla as dettagliobolla ";
        $SQL .= "FROM cp_dettagliofattura b LEFT OUTER JOIN cp_prodotto p ON (p.id = b.prodotto) LEFT OUTER JOIN cp_gruppoprodotti g ON (g.id = b.gruppo) ";
        ...
        while ($row = mysqli_fetch_array($result)) {
            ...
            $s .= "<cell>" . xml_entities($row['fattura']) . "</cell>";
            ...
            $s .= "<cell>" . xml_entities($row['totale']) . "</cell>";
            $s .= "<cell>" . xml_entities($row['descrizione']) . "</cell>";
            $s .= "<cell>" . xml_entities($row['bolla']) . "</cell>";
            $s .= "<cell>" . xml_entities($row['dettagliobolla']) . "</cell>";
            $s .= "</row>";
        }
        $s .= "</rows>";
        echo $s;
    ...
    case 21: // INSERT
        ...
        $prezzo = numberOrNull($_REQUEST["prezzounitario"]);
        $unmis = quoteStr(($_REQUEST['unitamisura']));
        $annotazioni = quoteStr(($_REQUEST['annotazioni']));

        $qrystr = "INSERT INTO cp_dettagliolistino (listino, prodotto, unitamisura, prezzounitario,annotazioni) ";
        $qrystr .= "VALUES ";
        $qrystr .= "($idriga, $prodotto, $unmis, $prezzo, $annotazioni)";
        $msg .= "\n$qrystr";
    ...
    case 22: // UPDATE
        ...
        $qrystr = "UPDATE cp_dettagliolistino SET prodotto = $prodotto, unitamisura = $unmis, ";
    ...
    case 23: // DELETE
        ...
        $id = $_REQUEST['id'];
        $qrystr = "DELETE FROM cp_dettagliolistino WHERE id = $id";
        $msg .= "\n$qrystr";
        $result = mysqli_query($db, $qrystr);
    ...
    case 24: // CURRENT ROW - PAGE TEST
        ...
        break;
    case 50: // CSV
        $result = mysqli_query($db, "SELECT COUNT(*) AS count FROM cp_dettagliolistino b WHERE 1 = 1 " . $wh);
        if (!$result) {
        ...
        if ($start < 0)
            $start = 0;
        $SQL = "SELECT b.id as id, b.listino as listino, b.prodotto as prodotto, ";
        $SQL .= "p.descrizionebreve as nomeprodotto, b.unitamisura as unitamisura, b.prezzounitario as prezzounitario, b.Annotazioni as annotazioni ";
        $SQL .= "FROM cp_dettagliolistino b LEFT OUTER JOIN cp_prodotto p ON (p.id = b.prodotto) ";
        $SQL .= "WHERE 1 = 1 " . $wh . " ORDER BY " . $sidx . " " . $sord ; // . " LIMIT " . $start . " , " . $limit;
