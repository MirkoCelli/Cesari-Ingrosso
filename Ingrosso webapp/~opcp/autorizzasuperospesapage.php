...
switch ($examp) {
    ...
    case 5: // per ottenere i dati del cp_ordine (per ottenere i dati delle combobox quando serve)
        $result = mysqli_query($db,"SELECT b.id as id, c.NomeBreve as cliente, b.dataordine as dataordine  FROM cp_ordinecliente b LEFT OUTER JOIN cp_cliente c ON (c.id = b.cliente) ORDER BY id ");
        if (!$result){
    ...
    case 6: // per ottenere i dati del cp_cliente
    ...
    case 7: // per ottenere i dati del cp_responsabile
        $result = mysqli_query($db, "SELECT id, CodiceIntermediario, Denominazione FROM cp_intermediario b ORDER BY Denominazione ");
        if (!$result) {
            header($_SERVER['SERVER_PROTOCOL'] . ' 500 Internal Server Error :: ' . "Query MySQL Error (6): " . mysqli_error($db), true, 500);
            echo ("Error description: " . mysqli_error($db));
    ...
    case 10:  // SELECT
        $result = mysqli_query($db,"SELECT COUNT(*) AS count FROM cp_autorizzazionisuperospesa b WHERE 1 = 1 ".$wh);
        if (!$result){
        ...
        if ($start<0) $start = 0;
        $SQL = "SELECT b.id AS id, b.ordine AS ordine, c.NomeBreve AS nomecliente, o.dataordine  AS dataordine, b.cliente AS cliente, ";
        $SQL .= "b.datarichiesta AS datarichiesta, b.responsabile AS responsabile, r.NomeBreve AS nomeresponsabile,  b.limiteconcesso AS limiteconcesso, ";
        $SQL .= "b.dataautorizzazione AS dataautorizzazione, b.annotazioni AS annotazioni ";
        $SQL .= "FROM cp_autorizzazionisuperospesa b LEFT OUTER JOIN cp_cliente c ON (c.id = b.cliente) ";
        $SQL .= "LEFT OUTER JOIN cp_ordinecliente o ON (o.id = b.ordine) ";
        ...
        while ($row = mysqli_fetch_array($result)){
            ...
            $s .= "<cell>" . xml_entities($row['datarichiesta']) . "</cell>";
            $s .= "<cell>" . xml_entities($row['responsabile']) . "</cell>";
            $s .= "<cell>" . xml_entities($row['nomeresponsabile']) . "</cell>";
            $s .= "<cell>" . xml_entities($row['limiteconcesso']) . "</cell>";
            $s .= "<cell>" . xml_entities($row['dataautorizzazione']) . "</cell>";
