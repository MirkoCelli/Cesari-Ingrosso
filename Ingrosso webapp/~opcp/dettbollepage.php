...
switch ($examp) {
    ...
    case 10:  // SELECT
        ...
        $sqlconta = "SELECT COUNT(*) AS count FROM cp_dettagliobolla b WHERE 1 = 1 " . $wh;
        if (isset($idordine) && ($idordine != -1) && ($idordine != "undefined")){
            $sqlconta = "SELECT COUNT(*) AS count FROM cp_dettagliobolla b JOIN cp_bollaconsegna c ON (b.bolla = c.id) WHERE c.ordine = " . $idordine . " " . $wh;
        }
        ...
        $SQL = "SELECT b.id as id, b.bolla as bolla, b.dettagliordine as sequenza, "; // attenzione in dettagliobolla il nome ha solo una o e non due
        $SQL .= "b.gruppo as gruppo, g.NomeGruppo as nomegruppo, b.prodotto as prodotto, p.descrizionebreve as nomeprodotto, ";
        $SQL .= "b.quantita as quantita, b.prezzounitario as prezzo, b.totale as totale, b.fattura as fattura ";
        $SQL .=	"FROM cp_dettagliobolla b JOIN cp_bollaconsegna c ON (c.id = b.bolla) LEFT OUTER JOIN cp_prodotto p ON (p.id = b.prodotto) LEFT OUTER JOIN cp_gruppoprodotti g ON (g.id = b.gruppo) ";

        if (isset($idordine) && ($idordine != -1) && ($idordine != "undefined")){
            $SQL .= "WHERE c.ordine = " . $idordine . " ";
        } else {
            $SQL .= "WHERE 1 = 1 ";
        }

        $SQL .= $wh." ORDER BY ".$sidx." ". $sord." LIMIT ".$start." , ".$limit;
