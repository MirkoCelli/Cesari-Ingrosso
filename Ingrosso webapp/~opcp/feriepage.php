...
if($searchOn=='true') {
    ...
    if( $fld=='id' || $fld =='annotazioni' || $fld=='datainizio' || $fld=='datafine' ) {
...
switch ($examp) {
    ...
    case 10:  // SELECT
        ...
        if ($start<0) $start = 0;
        $SQL = "SELECT b.id AS id, b.DataInizio AS datainizio, b.DataFine AS datafine,";
		$SQL .= "b.Annotazioni AS annotazioni ";
        $SQL .= "FROM cp_ferie b ";
    ...
    case 21:  // INSERT
        ...
        $qrystr = "INSERT INTO cp_ferie (datainizio, datafine, annotazioni) ";
        $qrystr .= "VALUES ";
        $qrystr .= "($datainizio, $datafine, $annotazioni)";
        $msg .= "\n$qrystr";
    ...
    case 22:	// UPDATE
        ...
        $datafine = DateFormatted($_REQUEST["datafine"], "dd/mm/yyyy");
        $annotazioni = quoteStr(($_REQUEST['annotazioni']));
        $qrystr = "UPDATE cp_ferie SET ";
        $qrystr .= "datainizio = $datainizio, datafine = $datafine, Annotazioni = $annotazioni ";
        $qrystr .= "WHERE id = $id";
        $msg .= "\n$qrystr";
        $result = mysqli_query($db,$qrystr);
        if (!$result){
    ...
    case 23:	// DELETE
        ...
        $id = $_REQUEST['id'];
        $qrystr = "DELETE FROM cp_ferie WHERE id = $id";
        $msg .= "\n$qrystr";
        $result = mysqli_query($db, $qrystr);
    ...
    case 50:  // CSV
        ...
        header("Content-Disposition: attachment; filename=ferie.csv");
