...
// INTERMEDIARI - 2024-06-06

// Elenco dei valori ammessi per Tipo Intermediario
var elencoTipo = ":;"; // indica un valore null di default se è NULLABLE in campo di destinazione
$.ajax({
    type: "GET",
    url: "intermediaripage.php?q=5",
    dataType: "xml",
...
jQuery("#navgridClienti").jqGrid({
    url:'intermediaripage.php?q=10',
    datatype: "xml",
    colNames:['ID','CODICE INTERMEDIARIO', 'DENOMINAZIONE','EMAIL','CODICE CLIENTE','TIPO INTERMEDIAZIONE','NOME INTERMEDIAZIONE','ANNOTAZIONE','DATA INIZIO','DATA FINE'],
    colModel:[
        ...
        {name:'codiceintermediario',index:'codiceintermediario', width:100, align:"left",editable:true,editoptions:{size:15}},
        { name: 'denominazione', index: 'denominazione', width: 250, editable: true, editoptions: { size: 250 } },
        { name: 'email', index: 'email', width: 250, editable: true, editoptions: { size: 128 } },
        { name: 'codicecliente', index: 'codicecliente', width: 50, editable: true, editoptions: { size: 128 } },
        {name:'tipointermediazione', index: 'tipointermediazione', width: 100, editable: true, edittype: "select", editoptions: { value: elencoTipo } },
        {name:'nomeintermediario', index: 'nomeintermediario', width: 150, editable: false, editoptions: { size: 100 } },
        {name:'annotazione', index: 'annotazione', width: 430, editable: true, editoptions: { size: 1000 } },
...
{reloadAfterSubmit:true,mtype:"POST",url:"intermediaripage.php?q=23",
...
jQuery("#navgridClienti").jqGrid('navButtonAdd','#pagernavClienti',{
       ...
       onClickButton : function () { 
           jQuery("#navgridClienti").jqGrid('excelExport',{"url":"intermediaripage.php?q=50"});
       } 
});
...
// {},
   ...
   ,addParams: {
    ...
    editParams: { mtype: "POST", keys: true, url: "intermediaripage.php?q=22",
                  successfunc: function () {
                      var $self = $(this);
                      setTimeout(function () {
                           $self.trigger("reloadGrid");
