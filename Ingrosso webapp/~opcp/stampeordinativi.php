...
$dataimb = $adesso; // 2024-09-07 manca la data imballaggio
...
*/
// 2024-08-28 - deve avere la bollaconsegna associata per emettere la stampa e avere importo bolla maggiore di zero
$sql = "SELECT o.id AS idordine, c.sequenza as sequenza, C.id AS idcliente, c.Denominazione AS nomecliente, o.ticket AS numeroticket, YEAR(o.dataordine) AS annocomp ";
$sql .= ", (p.perc_b % 123456) / 1000 as perc_b, (p.perc_n % 123456) / 1000 as perc_n "; // 2024-09-05
$sql .= "FROM cp_cliente c JOIN cp_ordinecliente o ON (o.cliente = c.id) JOIN cp_bollaconsegna b ON (b.dataconsegna = DATE('" . $giorno . "') AND (b.ordine = o.id OR b.ordine IS NULL)) JOIN cp_rapportoconsegna p ON (p.cliente = c.id) WHERE ";
$sql .= "o.dataordine = DATE('" . $giorno . "') ORDER BY c.sequenza";
...
while ($row1 = mysqli_fetch_array($result1)) {
    ...
    // 2024-09-05 - anche le percentuali per i calcoli dei totali
    $perc_b = $row1["perc_b"];
    $perc_n = $row1["perc_n"];
    $totali = CalcolaTotaliRapportati($idordine, $perc_b, $perc_n);
    // non deve stampare le bolle di importo zero - 2024-09-05
    if ($totali["totale_b"] + $totali["totale_n"] == 0.00){
       continue;
    }
    // fine 2024-09-05
...
// *********************************************** *
    ...
    $pdf->Text(85, 190, 'Data:' . $adesso); // 200
    */
// 05/08/2024
...
// fine 05/08/2024
    ...
    if ($perc_b < 100){ // solo se c'è un rapporto diverso da 100:0
        $pdf->SetFont('helvetica', 'B', 8); // grassetto, 12
