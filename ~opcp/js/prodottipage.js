...
var elencoGruppo = ":;"; // indica un valore null di default se è NULLABLE in campo di destinazione
$.ajax({
    type: "GET",
    url: "prodottipage.php?q=5",
    dataType: "xml",
    success: function(xml) {
       // abbiamo in xml il nostro file zona, ora lo dobbiamo assegnare a elencoZone
       elencoGruppo += $(xml).find('rows').text();
    },
    async:   false
});
...
jQuery("#navgridClienti").jqGrid({
    url:'prodottipage.php?q=10',
    datatype: "xml",
    colNames:['ID','CODICE PRODOTTO', 'GRUPPO','NOME GRUPPO','DESCRIZIONE BREVE','DESCRIZIONE COMPLETA','UNITA MISURA','SEQUENZA'],
    colModel:[
        ...
        { name: 'codiceprodotto', index: 'codiceprodotto', width: 120, align: "left", editable: true, editoptions: { size: 15 } },
        { name: 'gruppo', index: 'gruppo', width: 70, editable: true, edittype: "select", editoptions: { value: elencoGruppo } },
        { name: 'nomegruppo', index: 'nomegruppo', width: 150, editable: false, editoptions: { size: 100 } },        
        { name: 'descrizionebreve', index: 'descrizionebreve', width: 250, editable: true, editoptions: { size: 30 } },
        { name: 'descrizionecompleta', index: 'descrizionecompleta', width: 450, editable: true, editoptions: { size: 30 } },
        { name: 'unitamisura', index: 'unitamisura', width: 110, editable: true, editoptions: { size: 30 } },
        { name: 'sequenza', index: 'sequenza', width: 110, editable: true, editoptions: { size: 30 } }],
    rowNum: 25,
...
{reloadAfterSubmit:true,mtype:"POST",url:"prodottipage.php?q=23",
...
jQuery("#navgridClienti").jqGrid('navButtonAdd','#pagernavClienti',{
       ...
       onClickButton : function () { 
           jQuery("#navgridClienti").jqGrid('excelExport',{"url":"prodottipage.php?q=50"});
       } 
});
...
// {},
   ...
   ,addParams: {
    ...
    editParams: { mtype: "POST", keys: true, url: "prodottipage.php?q=22",
                  successfunc: function () {
                      var $self = $(this);
                      setTimeout(function () {
                           $self.trigger("reloadGrid");
