...
jQuery("#navgridClienti").jqGrid({
    ...
    datatype: "xml",
    colNames: ['ID', 'CODICE CLIENTE', 'DENOMINAZIONE', 'NOME BREVE', 'LISTINO', 'TIPO LISTINO', 'INTERMEDIARIO', 'NOME INTERMEDIARIO', 'ANNOTAZIONI', 'EMAIL','RIF.WHATSAPP',
               'SEQUENZA','P_B' , 'P_N','ABILITATO'],
    colModel:[
        ...
        {name:'nomeintermediario', index: 'nomeintermediario', width: 150, editable: false, editoptions: { size: 100 } },
        { name: 'annotazioni', index: 'annotazioni', width: 260, editable: true, editoptions: { size: 1000 } },
        { name: 'email', index: 'annotazioni', width: 260, editable: true, editoptions: { size: 1000 } },
        { name: 'rifwhatsapp', index: 'rifwhatsapp', width: 260, editable: true, editoptions: { size: 1000 } },
        { name: 'sequenza', index: 'sequenza', width: 60, align: "left", editable: true, editoptions: { size: 15 }, formatter: 'number', formatoptions: { decimalSeparator: ",", thousandsSeparator: "", decimalPlaces: 2 } },
        { name: 'p_b', index: 'p_b', width: 60, align: "left", editable: true, editoptions: { size: 15 }, formatter: 'number', formatoptions: { decimalSeparator: ",", thousandsSeparator: "", decimalPlaces: 3, defaultValue: '100.000' } },
        { name: 'p_n', index: 'p_n', width: 60, align: "left", editable: false, editoptions: { size: 15 }, formatter: 'number', formatoptions: { decimalSeparator: ",", thousandsSeparator: "", decimalPlaces: 3, defaultValue: '0.000' } },
        { name: 'abilitato', index: 'abilitato', width: 50, editable: true, edittype: "select", editoptions: { value: elencoAbilitato } }],
    rowNum: 25,
...
// Bottone per ottenere il codice del cliente selezionato

jQuery("#navgridClienti").jqGrid('navButtonAdd', '#pagernavClienti', {
    caption: "Codice",
    onClickButton: function () {
        alert("Codice cliente = " + jQuery("#navgridClienti").jqGrid('getGridParam', 'selrow'));
    }
});


// Bottone per ottenere il rapporto di consegna

jQuery("#navgridClienti").jqGrid('navButtonAdd', '#pagernavClienti', {
    caption: "../..",
    onClickButton: function () {
        var urlRapp = "clientipage.php?q=43&id=" + jQuery("#navgridClienti").jqGrid('getGridParam', 'selrow');
        $.ajax({
            type: "GET",
            url: urlRapp,
            dataType: "json",
            success: function (data) {
                // abbiamo in data il nostro rapporto di consegna
                alert(data.cliente + "=" + data.perc_b + " : " + data.perc_n);
            },
            async: false
        });
    }
