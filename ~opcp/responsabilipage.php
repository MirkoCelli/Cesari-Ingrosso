...
if($searchOn=='true') {
    ...
    if( $fld=='id' || $fld =='nomebreve' || $fld=='nomecompletoresponsabile' || $fld=='datainizio' || $fld=='datafine' || $fld=='datainserimento' || $fld=='email') {
        ...
        if ($fld == 'nomecompletoresponsabile') {
            $fld = 'b.NomeCompeltoResponsabile';
        }
        if ($fld == 'email') {
            $fld = 'b.email';
        }
        ...
        if ($fld == 'datainserimento') {
            $fld = 'b.datainserimento';
        }
        // costruct where
...
switch ($examp) {
    ...
    case 10:  // SELECT
        $result = mysqli_query($db,"SELECT COUNT(*) AS count FROM cp_responsabile b WHERE 1 = 1 ".$wh);
        if (!$result){
        ...
        if ($start<0) $start = 0;
        $SQL = "SELECT b.id as id, b.NomeBreve as nomebreve, b.NomeCompletoResponsabile as nomecompletoresponsabile, b.email AS email, ";
        $SQL .= "b.datainizio as datainizio, b.datafine as datafine, b.datainserimento as datainserimento ";
        $SQL .=	"FROM cp_responsabile b  ";
    ...
    case 21:  // INSERT
        ...
        $nomebreve = quoteStr(strtoupper($_REQUEST['nomebreve']));
        $nomecompletoresponsabile = quoteStr(($_REQUEST['nomecompletoresponsabile']));
        ...
        // $datainserimento = quoteStr(($_REQUEST['inserimento'])); // si intende adesso
        $datainserimento = quoteStr(DATE("Y-m-d H:i:s"));

        $qrystr = "INSERT INTO cp_responsabile (NomeBreve,NomeCompletoResponsabile,datainizio,datafine,datainserimento,email) ";
        $qrystr .= "VALUES ";
        $qrystr .= "($nomebreve, $nomecompletoresponsabile, $datainizio, $datafine, $datainserimento, $email)";
        $msg .= "\n$qrystr";
    ...
    case 22:	// UPDATE
        ...
        $qrystr = "UPDATE cp_responsabile SET NomeBreve = $nomebreve, NomeCompletoResponsabile = $nomecompletoresponsabile, email = $email , ";
        $qrystr .= "datainizio = $datainizio, datafine = $datafine ";
        $qrystr .= "WHERE id = $id";
        $msg .= "\n$qrystr";
        $result = mysqli_query($db,$qrystr);
        if (!$result){
    ...
    case 23:	// DELETE
        ...
        $id = $_REQUEST['id'];
        $qrystr = "DELETE FROM cp_responsabile WHERE id = $id";
        $msg .= "\n$qrystr";
        $result = mysqli_query($db,$qrystr);
