...
if($searchOn=='true') {
    ...
    if( $fld=='id' || $fld =='codiceintermediario' || $fld=='denominazione' || $fld=='tipointermediazione' || $fld=='annotazione'
        || $fld=='datainizio' || $fld=='datafine' || $fld == 'email' || $fld == 'codicecliente') {
        ...
        if ($fld == 'codiceintermediario') {
            $fld = 'b.CodiceIntermediario';
        }
        ...
        if ($fld == 'tipointermediazione') {
            $fld = 'b.tipoIntermediazione';
        }
        if ($fld == 'annotazione') {
            $fld = 'b.Annotazione';
        }
...
switch ($examp) {
    ...
    case 5: // per ottenere i dati del cp_listino (per ottenere i dati delle combobox quando serve)
        $result = mysqli_query($db,"SELECT id, tipo FROM cp_tipointermediario b ORDER BY id ");
        if (!$result){
    ...
    case 10:  // SELECT
        $result = mysqli_query($db,"SELECT COUNT(*) AS count FROM cp_intermediario b WHERE 1 = 1 ".$wh);
        if (!$result){
        ...
        if ($start<0) $start = 0;
        $SQL = "SELECT b.id as id, b.CodiceIntermediario as codiceintermediario, b.Denominazione as denominazione, b.email as email, ";
        $SQL .= "b.tipoIntermediazione as tipointermediazione, l.tipo as nomeintermediazione, b.Annotazione as annotazione, b.datainizio as datainizio, b.datafine as datafine, b.codcliente as codicecliente ";
        $SQL .=	"FROM cp_intermediario b LEFT OUTER JOIN cp_tipointermediario l ON (l.id = b.tipoIntermediazione) ";
        ...
        while ($row = mysqli_fetch_array($result)){
            ...
            $s .= "<cell>" . xml_entities($row['codiceintermediario']) . "</cell>";
            ...
            $s .= "<cell>" . xml_entities($row['nomeintermediazione']) . "</cell>";
            $s .= "<cell>" . xml_entities($row['annotazione']) . "</cell>";
    ...
    case 21:  // INSERT
        ...
        $codiceintermediario = quoteStr(strtoupper($_REQUEST['codiceintermediario']));
        $denominazione = quoteStr(($_REQUEST['denominazione']));
        $email = quoteStr(($_REQUEST['email']));
        $codicecliente = numberOrNull($_REQUEST["codicecliente"]);
        $tipointermediazione = numberOrNull($_REQUEST['tipointermediazione']);
        $nomeintermediazione = quoteStr(($_REQUEST['nomeintermediazione']));
        $annotazione = quoteStr(($_REQUEST['annotazione']));
        $datainizio = (DateFormatted($_REQUEST['datainizio'],"dd/mm/yyyy"));
        $datafine = (DateFormatted($_REQUEST['datafine'],"dd/mm/yyyy"));

        $qrystr = "INSERT INTO cp_intermediario (CodiceIntermediario, Denominazione, email, tipoIntermediazione, Annotazione, datainizio, datafine, codcliente) ";
        $qrystr .= "VALUES ";
        $qrystr .= "($codiceintermediario, $denominazione, $email, $tipointermediazione, $annotazione, $datainizio, $datafine,$codicecliente)";
        $msg .= "\n$qrystr";
    ...
    case 22:	// UPDATE
        ...
        $qrystr = "UPDATE cp_intermediario SET CodiceIntermediario = $codiceintermediario, Denominazione = $denominazione, email = $email, ";
        $qrystr .= "tipoIntermediazione = $tipointermediazione, Annotazione = $annotazione, datainizio = $datainizio, datafine = $datafine , codcliente = $codicecliente ";
        $qrystr .= "WHERE id = $id";
        $msg .= "\n$qrystr";
        $result = mysqli_query($db,$qrystr);
        if (!$result){
    ...
    case 23:	// DELETE
        ...
        $id = $_REQUEST['id'];
        $qrystr = "DELETE FROM cp_intermediario WHERE id = $id";
        $msg .= "\n$qrystr";
        $result = mysqli_query($db,$qrystr);
    ...
    case 50:  // CSV
        ...
        if ($start<0) $start = 0;
        $SQL = "SELECT b.id as id, b.CodiceIntermediario as codiceintermediario, b.Denominazione as denominazione, b.email as email, ";
        $SQL .= "b.tipoIntermediazione as tipointermediazione, l.tipo as nomeintermediazione, b.Annotazione as annotazione, b.datainizio as datainizio, b.datafine as datafine, b.codcliente as codiccliente ";
        $SQL .= "FROM cp_intermediario b LEFT OUTER JOIN cp_tipointermediario l ON (l.id = b.tipoIntermediazione) ";
        $SQL .= "WHERE 1 = 1 " . $wh . " ORDER BY " . $sidx . " " . $sord; // . " LIMIT " . $start . " , " . $limit;
