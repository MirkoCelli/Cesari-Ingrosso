<?php
// MOSTRA ORDINE DA TICKET PER ANNO COMPETENZA - 2024.07.08
/* Qui carichiamo i dati dell'ordine legato al ticket per l'anno di competenza */
...
if (isset($_REQUEST["annocomp"])) {
    $annocomp = $_REQUEST["annocomp"];
}
// informazioni sull'ordine associato al ticket nell'anno di competenza
...
<?php
  if ($idordine == null){
?>
      Non sono state trovate informazioni per il Ticket <?= $ticket ?>.
<?php      
  } else {
?>      
...
?>
                ...
                $sql = "SELECT o.id AS id, o.ticket as ticket, o.preparatore as preparatore, r.NomeBreve as nomeresponsabile, o.stato as stato ";
                $sql .= "FROM ";
                ...
                if ($row = mysqli_fetch_array($result)) {
                    ...
                    $statooper = $row["stato"];
                    if (($idpreparatore == $responsabile) && ($statooper == 7)){
                    ...
                    <td><button name="stampa" onclick="return StampaOrdine(<?=$idordine?>);">Stampa</button></td>
                </tr>
                <?php
  }
  mysqli_free_result($result);
