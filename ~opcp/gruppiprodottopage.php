...
if($searchOn=='true') {
    ...
    if( $fld=='id' || $fld =='nomegruppo' || $fld=='descrizione') {
...
switch ($examp) {
    ...
    case 10:  // SELECT
        $result = mysqli_query($db,"SELECT COUNT(*) AS count FROM cp_gruppoprodotti b WHERE 1 = 1 ".$wh);
        if (!$result){
        ...
        if ($start<0) $start = 0;
        $SQL = "SELECT b.id as id, b.NomeGruppo as nomegruppo, b.Descrizione as descrizione ";
        $SQL .=	"FROM cp_gruppoprodotti b  ";
    ...
    case 21:  // INSERT
        ...
        $nomegruppo = quoteStr(strtoupper($_REQUEST['nomegruppo']));
        $descrizione = quoteStr(($_REQUEST['descrizione']));

        $qrystr = "INSERT INTO cp_gruppoprodotti (NomeGruppo, Descrizione) ";
        $qrystr .= "VALUES ";
        $qrystr .= "($nomegruppo, $descrizione)";
        $msg .= "\n$qrystr";
    ...
    case 22:	// UPDATE
        ...
        $qrystr = "UPDATE cp_gruppoprodotti SET NomeGruppo = $nomegruppo, Descrizione = $descrizione ";
        $qrystr .= "WHERE id = $id";
        $msg .= "\n$qrystr";
        $result = mysqli_query($db,$qrystr);
        if (!$result){
    ...
    case 23:	// DELETE
        ...
        $id = $_REQUEST['id'];
        $qrystr = "DELETE FROM cp_gruppoprodotti WHERE id = $id";
        $msg .= "\n$qrystr";
        $result = mysqli_query($db,$qrystr);
