...
if($searchOn=='true') {
    ...
    if( $fld=='id' || $fld =='descrizionestato' || $fld=='annotazioni') {
        ...
        if ($fld == 'descrizionestato') {
            $fld = 'b.descrizionestato';
        }
        if ($fld == 'annotazioni') {
            $fld = 'b.Annotazioni';
        }
...
switch ($examp) {
    ...
    case 10:  // SELECT
        $result = mysqli_query($db,"SELECT COUNT(*) AS count FROM cp_statoordine b WHERE 1 = 1 ".$wh);
        if (!$result){
        ...
        if ($start<0) $start = 0;
        $SQL = "SELECT b.id as id, b.descrizionestato as descrizionestato, b.Annotazioni as annotazioni ";
        $SQL .=	"FROM cp_statoordine b ";
    ...
    case 21:  // INSERT
        ...
        $descrizionestato = quoteStr(strtoupper($_REQUEST['descrizionestato']));
        $annotazioni = quoteStr(($_REQUEST['annotazioni']));

        $qrystr = "INSERT INTO cp_statoordine (descrizionestato, annotazioni) ";
        $qrystr .= "VALUES ";
        $qrystr .= "($descrizionestato, $annotazioni)";
        $msg .= "\n$qrystr";
    ...
    case 22:	// UPDATE
        ...
        $qrystr = "UPDATE cp_statoordine SET descrizionestato = $descrizionestato, Annotazioni = $annotazioni ";
        $qrystr .= "WHERE id = $id";
        $msg .= "\n$qrystr";
        $result = mysqli_query($db,$qrystr);
        if (!$result){
    ...
    case 23:	// DELETE
        ...
        $id = $_REQUEST['id'];
        $qrystr = "DELETE FROM cp_statoordine WHERE id = $id";
        $msg .= "\n$qrystr";
        $result = mysqli_query($db,$qrystr);
