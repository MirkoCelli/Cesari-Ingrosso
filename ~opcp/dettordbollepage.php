...
// DETTAGLIO ORDINI PER BOLLE CONSEGNA
...
switch ($examp) {
    ...
    case 10:  // SELECT
        ...
        $idordine = -1; // non deve esistere
        if (isset($_REQUEST["idordine"])) {
            $idordine = $_REQUEST["idordine"];
        }
        $sqlconta = "SELECT COUNT(*) AS count FROM cp_dettaglioordine b WHERE b.stato = 0 ".$wh;
        if (isset($idordine) && ($idordine != -1) && ($idordine != "undefined")){
            $sqlconta = "SELECT COUNT(*) AS count FROM cp_dettaglioordine b WHERE b.stato = 0 AND b.id = " . $idordine . " " . $wh;
        }
        $result = mysqli_query($db,$sqlconta);
        if (!$result){
        ...
        $SQL = "SELECT b.id AS id, b.ordinecliente AS ordine, b.dettaglioordine AS sequenza, b.prodotto AS prodotto, p.descrizionebreve AS nomeprodotto, b.gruppo AS gruppo, ";
        $SQL .= "g.NomeGruppo AS nomegruppo, b.quantita AS quantita, b.unitamisura AS unitamisura, IFNULL(dl.prezzounitario,dg.prezzounitario) AS prezzo, IFNULL(b.quantita,0) * IFNULL(dl.prezzounitario,dg.prezzounitario) AS totale ";
        $SQL .= "FROM cp_dettaglioordine b JOIN cp_ordinecliente o ON (o.id = b.ordinecliente) JOIN cp_cliente c ON (c.id = o.cliente) JOIN cp_listinoprezzi l ON (l.id = c.listino) ";
        $SQL .= "LEFT OUTER JOIN cp_dettagliolistino dl ON (dl.listino = l.id AND dl.prodotto = b.prodotto) ";
        $SQL .= "LEFT OUTER JOIN cp_dettagliolistinogruppi dg ON (dg.listino = l.id AND dg.gruppo = b.gruppo) ";
        $SQL .= "LEFT OUTER JOIN cp_prodotto p ON (p.id = b.prodotto) ";
        $SQL .= "LEFT OUTER JOIN cp_gruppoprodotti g ON (g.id = b.gruppo) ";
        $SQL .= "WHERE b.stato = 0 ";
        // viene indicato anche l'ordine di rifefrimento
        if (isset($idordine) && ($idordine != -1) && ($idordine != "undefined")){
           $SQL .= "AND b.ordinecliente = " . $idordine . " ";
        }
        $SQL .= "ORDER BY b.dettaglioordine ";
        ...
        */
        ...
        while ($row = mysqli_fetch_array($result)){
            ...
            $s .= "<cell>" . xml_entities($row['prezzo']) . "</cell>";
