...
// 2024-10-14 - se la giornata è antecedente a oggi (adesso) allora mette la data odierna, perchè non permettiamo di navigare più indietro di oggi
if (substr($adesso,0,10) >= $giornata)
{
    $giornata = date("Y-m-d");
}
// fine 2024-10-14
...
// 2024-10-14 - mi serve lo stato abilitato del cliente per stabilire come visualizzare gli ordini dalla data odierna in poi
...
$sql .= " AND l.id = " . $indice; // manca il controllo del vincolo per l'utente
...
?>
                            <input type="submit" name="prec" value="< PRECEDENTE " class="bottonedata" />
<?php
   }
?>
                        </form>
                    ...
                    <td align="center" class="intestac">
                        <form method="post" action="settimanale.php" name="chgdate" id="chgdate">
                            <span id="settimana">
                                ...
                                <input type="submit" name="invia" value="Cambia Data" class="bottonedata" />
                            </span>
                        </form>
                    </td>
                    ...
                    <td align="right" class="intesta">
                        <form method="post" action="settimanale.php">
                            ...
                            <input type="submit" name="succ" value="SUCCESSIVO >" class="bottonedata" />
                        </form>
                    </td>

                    <td align="center" class="intesta2">
                        <a href="<?= $serverpath ?>mainpage.php" class="bottonehome">HOME</a>
                    </td>
                </tr>
            </table>
    </div>
  ...
  // elenchiamo i giorni della settimana: da $datainiziale a $datafinale
     ...
     foreach ($period as $dt) {
        ...
        if ( $indgiorno > 1 ){
            // 2024-09-07 - solo se la data $giorno1 è non precedente a $datacontratto e $datapmp allor si mostra il link e deve essere abilitato
            if ($giorno1 >= $datacontratto && $giorno1 >= $datapmp && !DataInChiusuraFerie($giorno1) && $abilitato == 1){
                    ?>
                    <td class="date">
                        <a href="<?=$serverpath?>giornaliero.php?giorno=<?=$giorno1?>" class="bottone"><?= $giorno ?></a>
                    </td>
<?php       }  else { // fine 2024-09-07
                    ?>
                    <td class="date">
                       <!-- <a href="<?=$serverpath?>giornaliero.php?giorno=<?=$giorno1?>" class="bottone"> -->
                        <?= $giorno ?>
                        <!--</a>-->
                    </td>
<?php       }
...
?>
  ...
  // qui cerco per il cliente associato a indice i dati per la settimana
      ...
      $sql .= "WHERE l.id = " . $indice; // 03/03/2025 corretto errore mancava indice da vincolare al solo utente
      ...
      foreach ($period as $dt) {
         ...
         // 2024-10-14 - se è disabilitato oppure il giorno è nel periodo di ferie allora non elenca i prodotti dell'ordine
         if (($giorno1 >= substr($adesso,0,10) && $abilitato == 0) || (DataInChiusuraFerie($giorno1))){
  ...
  <td valign="top">
   <table class="dettagli">
       <tr>
          <td class="dettagli">U.M.</td>
          <td class="dettagli">Q.t&agrave;</td>
          <td class="dettagli">Prodotto</td>
          <td class="dettagli">Gruppo</td>
      </tr>
<?php
...
?>
      <tr>
          <td class="dettagli" align="right"><?=$row["unitamisura"]?></td>
          <td class="dettagli" align="right"><?=RiduciNumero($row["quantita"])?></td>
          <td class="dettagli" align="left"><?=$row["nomeprodotto"]?></td>
          <td class="dettagli" align="right"><?=$row["gruppo"]?></td>
...
<!-- 2024-08-15 - tabellina di riepilogo per gruppi prodotto dell'ordinato di questo giorno particolare per questo cliente -->
      ...
      <table class="dettagli1">
          ...
          <?php
          // 2024-10-14 - anche il raggruppamento va fatto solo se siamo abilitati e in giorni non di chiusura ferie
              if (($giorno1 >= substr($adesso, 0, 10) && $abilitato == 0) || (DataInChiusuraFerie($giorno1))) {
                  $sql = "SELECT * FROM cp_ordinecliente WHERE false";
          ...
          ?>
             <tr>
                 <td><?= $row1["nomegruppo"] ?></td>
                 <td><?= $row1["quantita"] ?></td>
                 <td><?= $row1["unitamisura"] ?></td>
             </tr>
<?php
           }
