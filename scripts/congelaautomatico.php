...
// 2024-10-14 - non deve essere un giorno nel periodo di ferie il giornoseguente

$sql1 = "SELECT COUNT(*) AS conta FROM cp_ferie WHERE DataInizio <= DATE('" . $giornodopo . "') AND DataFine >= DATE('" . $giornodopo . "') ";
...
// se conta è > 0 allora finisce qui perchè equivale a lunedì
if ($row = mysqli_fetch_array($result1)){
	if ($row["conta"] > 0){
	  header("Content-type: application/json; charset=utf-8");
      echo "{\"status\" : \"KO\" , \"giorno\" : \" " . $giorno . "\" , \"error\" : \"Chiusura per Ferie " . $errore . "\"}";
      exit;
	}
}
...
$sql = "SELECT c.id AS c1_id, s.id AS s1_id, o.id as o1_id FROM cp_cliente c JOIN cp_schemadefault s ON (c.id = s.cliente AND s.datainizio <= DATE('" . $giornodopo . "') AND s.datafine IS NULL) ";
$sql .= "LEFT OUTER JOIN cp_ordinecliente o ON (o.cliente = c.id AND o.schematico = s.id AND o.dataordine = DATE('" . $giornodopo . "')) ";
$sql .= "WHERE c.abilitato = 1 "; // 2024-10-14 solo per i clienti abilitati si genera l'ordine // 2024-10-14
